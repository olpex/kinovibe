...
module.exports = async function handler(req, res) {
    ...
    if (!wishes) {
        return res.status(400).json({ error: 'Відсутнє поле: wishes' });
    }
    ...
    for (const model of MODELS) {
        ...
        try {
            ...
            if (result.status === 401) {
                return res.status(502).json({ error: `Недійсний Gemini API Key.` });
            }
            ...
            if (result.status !== 200) {
                ...
                if (!isFallbackable) return res.status(502).json({ error: msg });
                lastError = msg;
                continue;
            }
